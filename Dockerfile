...
RUN yum -y --enablerepo=remi,remi-php56 install httpd php php-pdo php-common php-cli php-pear php-mysqlnd php-pecl-mongo php-gd php-mbstring* php-mcrypt php-xml mongodb mongodb-server nodejs npm git deltarpm mod_ssl
...
RUN sed -i "s/^SSLCertificateFile.*/SSLCertificateFile \/etc\/pki\/tls\/certs\/ca.crt/g" /etc/httpd/conf.d/ssl.conf
RUN sed -i "s/^SSLCertificateKeyFile.*/SSLCertificateKeyFile \/etc\/pki\/tls\/private\/ca.key/g" /etc/httpd/conf.d/ssl.conf
